...
ADLA_TOOL_PATH=../bin/
#ADLA_TOOL_PATH=../python/tvm/
adla_convert=${ADLA_TOOL_PATH}adla_convert


if [ ! -e "$adla_convert" ]; then
    adla_convert=${ADLA_TOOL_PATH}adla_convert.py
fi
...
$adla_convert --model-type onnx \
        ...
        --outputs "310 315" #output name of a certain layer
...
$adla_convert --model-type mxnet \
        ...
        --quantize-dtype int8 --outdir mxnet_output \
        --channel-mean-value "0,0,0,256" \
...
$adla_convert --model-type tensorflow \
        ...
        --quantize-dtype uint8 --outdir tensorflow_output \
        --channel-mean-value "0,0,0,256" \
        --source-file dataset.txt \
        --target-platform PRODUCT_PID0XA001
...
$adla_convert --model-type tflite \
        ...
        --source-file dataset.txt \
        --target-platform PRODUCT_PID0XA001 \
        #--outputs '99 111' # View the model through netron, not the output name of the layer, but location: 111
...
$adla_convert --model-type coreml \
        --model ./model_source/coreml_model/Resnet50.mlmodel \
        --inputs "image" \
        --input-shapes  "3,224,224"  \
        --dtypes "float32" \
        --quantize-dtype int8 --outdir coreml_output  \
        --batch-size 1 --target-platform PRODUCT_PID0XA001
