...
class aml_nn_model:
    def __init__(self):
        ...
        self.quantize_algos=["min-max","percentile","hist_percentile","hist_asymm_percentile","moving-avg","history_moving-avg","kl_asymm_divergence","kl_symm_divergence"]
        ...
        self.kl_symm_optimize_algos = ["kl-dis", "mse-dis"]
        self.kl_asymm_optimize_algos = ["cos-dis", "kl-dis", "js-dis"]
                        ...
                        for weights_quantize_algo in aml_model.weights_quantize_algos:
                            if weights_quantize_algo=="mle":
                                ...
                                if ret:
                                    ...
                                    exit()
                                aml_model.test_result(cmd_line)
        elif  quantize_algo=="history_moving-avg" or quantize_algo=="moving-avg":
            for moving_alpha in aml_model.moving_alphas:
                for iterations in aml_model.iterations:
                    for weights_quantize_algo in aml_model.weights_quantize_algos:
                        if weights_quantize_algo=="mle":
                            for weights_thres_sizes in aml_model.weights_thres_sizes:
                                for weights_neg_scales in aml_model.weights_neg_scales:
                                    for weights_optimize_algos in aml_model.weights_optimize_algos:
                                        cmd_line=aml_model.cmd_line_base + " --weights-quantize-algo mle \
                                        --weights-threshold-size {} --iterations {} \
                                        --quantize-algo {} --moving-alpha {} \
                                        --weights-neg-scale {} --weights-optimize-algo {}".format(weights_thres_sizes,iterations,
                                                                                                    quantize_algo,moving_alpha,
                                                                                                    weights_neg_scales,weights_optimize_algos)
                                        print(cmd_line)
                                        ret=os.system(cmd_line)
                                        if ret:
                            ...
                            cmd_line=aml_model.cmd_line_base + " --quantize-algo {} --moving-alpha {} --iterations {}".format(quantize_algo,moving_alpha,iterations)
                            print(cmd_line)
                            ret=os.system(cmd_line)
                            if ret:
                                print("Model conversion command failed to execute,\nCommand：{}".format(cmd_line))
                                exit()
                            aml_model.test_result(cmd_line)
        elif  quantize_algo=="kl_symm_divergence":
            for optimize_algo in aml_model.kl_symm_optimize_algos:
                for num_histogram_bin in aml_model.num_histogram_bins:
                    for kl_thres_size in aml_model.kl_thres_sizes:
                        for iterations in aml_model.iterations:
                            for weights_quantize_algo in aml_model.weights_quantize_algos:
                                if weights_quantize_algo=="mle":
                                    for weights_thres_sizes in aml_model.weights_thres_sizes:
                                        for weights_neg_scales in aml_model.weights_neg_scales:
                                            for weights_optimize_algos in aml_model.weights_optimize_algos:
                                                cmd_line=aml_model.cmd_line_base + " --weights-quantize-algo mle \
                                                --weights-threshold-size {} --iterations {} \
                                                --quantize-algo {} --optimize-algo {} --kl-threshold-size {}\
                                                --weights-neg-scale {} --weights-optimize-algo {}".format(weights_thres_sizes,iterations,
                                                                                                        quantize_algo,optimize_algo,kl_thres_size,
                                                                                                        weights_neg_scales,weights_optimize_algos)
                                                print(cmd_line)
                                                ret=os.system(cmd_line)
                                                if ret:
                                    ...
                                    cmd_line=aml_model.cmd_line_base + " --quantize-algo {} --optimize-algo {}  --divergence-nbins {} --kl-threshold-size {} --iterations {}".format(quantize_algo,optimize_algo,num_histogram_bin,kl_thres_size,iterations)
                                    print(cmd_line)
                                    ret=os.system(cmd_line)
                                    if ret:
                                        print("Model conversion command failed to execute,\nCommand：{}".format(cmd_line))
                                        exit()
                                    aml_model.test_result(cmd_line)
        elif  quantize_algo=="kl_asymm_divergence":
            for optimize_algo in aml_model.kl_asymm_optimize_algos:
                for num_histogram_bin in aml_model.num_histogram_bins:
                    for kl_thres_size in aml_model.kl_thres_sizes:
                        for neg_scale in aml_model.neg_scales:
                            for iterations in aml_model.iterations:
                                for weights_quantize_algo in aml_model.weights_quantize_algos:
                                    if weights_quantize_algo=="mle":
                                        for weights_thres_sizes in aml_model.weights_thres_sizes:
                                            for weights_neg_scales in aml_model.weights_neg_scales:
                                                for weights_optimize_algos in aml_model.weights_optimize_algos:
                                                    cmd_line=aml_model.cmd_line_base + " --weights-quantize-algo mle \
                                                    --weights-threshold-size {} --iterations {} \
                                                    --quantize-algo {} --optimize-algo {} --kl-threshold-size {} --neg-scale {}\
                                                    --weights-neg-scale {} --weights-optimize-algo {}".format(weights_thres_sizes,iterations,
                                                                                                            quantize_algo,optimize_algo,kl_thres_size,neg_scale,
                                                                                                            weights_neg_scales,weights_optimize_algos)
                                                    print(cmd_line)
                                                    ret=os.system(cmd_line)
                                                    if ret:
                                    ...
                                    cmd_line=aml_model.cmd_line_base + " --quantize-algo {} --optimize-algo {} --divergence-nbins {} --kl-threshold-size {} --neg-scale {} --iterations {}".format(quantize_algo,optimize_algo,num_histogram_bin,kl_thres_size,neg_scale,iterations)
                                    print(cmd_line)
                                    ret=os.system(cmd_line)
                                    if ret:
                                        print("Model conversion command failed to execute,\nCommand：{}".format(cmd_line))
